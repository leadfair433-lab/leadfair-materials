...
export default function ContactPage(){
  return <>
    <section className="contact-hero shell"><div><span>CONTACT FOUND FAIR</span><h1>從需求開始，<br/>讓材料方案更快落地。</h1><p>告訴我們產品用途、目標效能與專案階段。材料工程師會整理關鍵問題，協助您完成選材、驗證與量產評估。</p><div className="hero-reply"><b>24h</b><span>工作日內由材料工程師直接對接</span></div></div><form id="inquiry" className="hero-quick-form" onSubmit={event=>{event.preventDefault();event.currentTarget.reset();alert("已收到您的需求，我們將儘快與您聯絡。");}}><div><span>PROJECT INQUIRY</span><h2>提交您的專案需求</h2><p>與首頁使用相同資訊，方便材料工程師統一跟進。</p></div><div className="field-row"><label>您的姓名<input name="name" required placeholder="請輸入姓名"/></label><label>公司名稱<input name="company" required placeholder="請輸入公司名稱"/></label></div><div className="field-row"><label>工作郵箱<input name="email" required type="email" placeholder="name@company.com"/></label><label>聯絡電話<input name="phone" required type="tel" placeholder="國家程式碼 + 電話號碼"/></label></div><div className="field-row"><label>目標材料<select name="material" defaultValue="IUS-4065"><option>IUS-4065</option><option>阻燃 TPE</option><option>生物基 TPE</option><option>其他定製材料</option></select></label><label>預計用量<input name="volume" placeholder="例如：5 噸 / 月"/></label></div><label>專案需求<textarea name="message" rows={3} placeholder="請描述應用場景、硬度、顏色、認證或交付要求"/></label><label className="consent"><input type="checkbox" required/> 我同意材料工程師就本次專案需求與我聯絡</label><button type="submit">提交免費樣品申請 <b>↗</b></button></form></section>
