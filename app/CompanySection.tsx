...
const slides = [
  "/images/company/company-story-taiwan-20260914-fast.jpg",
  "/images/company/generated/taiwan-entrance-fast.jpg",
  "/images/company/generated/history-campus-grid-fast.jpg",
];
...
export default function CompanySection() {
  ...
  const locale = pathname.match(/^\/(zh-tw|en|vi)(?:\/|$)/)?.[1];
  const laboratoryBase = `${process.env.NEXT_PUBLIC_BASE_PATH || ""}${locale ? `/${locale}` : ""}/company/laboratory`;
  ...
  return <section className="profile-page" id="company"><div className="shell profile-shell">
    ...
    <section className="profile-about" id="company-story"><div className="profile-copy"><span>ABOUT US</span><h2>臺灣峰暉塑膠工業股份有限公司</h2><p>臺灣峰暉塑膠工業股份有限公司於1979年成立於臺灣彰化縣，主要生產經營各類塑膠鞋材，包括各種 ABS、IPS、PC 射出鞋跟、TPR、PVC 及 PU 發泡鞋底，並持續投入材料研發與製程優化，以穩定的品質滿足全球客戶需求。</p><p>集團陸續於中國大陸設立多家生產與營運公司。歷經四十多年發展，秉持「追求卓越、止於至善」的經營理念，致力於創新、服務與永續經營，為客戶創造長期價值。</p><a href="#company-history">了解更多 <b>→</b></a></div><figure><img src="/images/company/company-story-taiwan-20260914-fast.jpg" alt="臺灣峰暉塑膠工業廠區正門"/><figcaption>「以材料創新推動產業進步，<br/>與客戶共創永續未來。」</figcaption></figure></section>
    ...
    <section className="profile-labs" id="company-laboratory"><header><div><span>LABORATORY ENVIRONMENT</span><h2>實驗室環境</h2></div><a className="profile-lab-all" href={`${laboratoryBase}/`}>查看全部實驗室儀器 <span aria-hidden="true">↗</span></a></header><div className="profile-lab-showcase"><div className="profile-lab-intro"><span>FF / LF LABORATORY</span><h3>從配方開發到性能驗證，設備各自清晰呈現</h3><p>每張圖片連至獨立介紹頁，日後可逐篇補上設備名稱、用途與測試資訊。</p><a className="profile-lab-all" href={`${laboratoryBase}/`}>瀏覽實驗室儀器總覽 <span aria-hidden="true">↗</span></a></div><div className="profile-lab-gallery">{laboratoryArticles.map((article,index)=><figure key={article.slug}><a href={`${laboratoryBase}/${article.slug}/`} aria-label={`閱讀介紹：${article.title}`}><img src={article.image} alt={article.title}/><span aria-hidden="true">↗</span></a><figcaption><b>{String(index+1).padStart(2,"0")}</b><span>{article.title}</span><a href={`${laboratoryBase}/${article.slug}/`}>查看詳情 <span aria-hidden="true">↗</span></a></figcaption></figure>)}</div></div></section>
  </div></section>;
}
