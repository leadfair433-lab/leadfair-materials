...
function Blocks({ blocks }: { blocks: ArticleBlock[] }) {
  return <>{blocks.map((block, i) => {
    ...
    if (block.type === "table") return <div className="journal-table-scroll" key={i} role="region" aria-label="技術資料表" tabIndex={0}><table><tbody>{block.rows?.map((row, r) => <tr key={r}>{row.map((cell, c) => r === 0 ? <th key={c} colSpan={cell.colSpan}>{cell.text}</th> : <td key={c} colSpan={cell.colSpan}>{cell.text}</td>)}</tr>)}</tbody></table></div>;
    ...
    <article className="journal-sheet">
      ...
      <div className="journal-content">
        <div className="journal-hero">
          <div><span className="journal-category">{article.category}</span><h1>{article.title}</h1>{article.englishTitle && <p>{article.englishTitle}</p>}<p>{article.summary}</p><span className="journal-series">材料洞察 / 應用指南{article.illustrativeCover && " · 主圖為 AI 情境示意，非實測照片"}</span></div>
          <img src={articleAssetPath(article.image)} alt={article.alt} width="1536" height="1024" />
        </div>
        <div className="journal-reading">
          <aside><nav className="journal-outline" aria-label="文章目錄"><h2>OUTLINE / 文章大綱（點選跳轉）</h2>{article.sections.map((section, index) => <a key={section.title} href={`#section-${index + 1}`}><span>{String(index + 1).padStart(2, "0")}.</span>{section.title}</a>)}</nav><ArticleShare title={article.title} /><ArticleContact /></aside>
          <div className="journal-body">{article.sections.map((section, index) => <section id={`section-${index + 1}`} key={section.title}><h2><span>{String(index + 1).padStart(2, "0")}</span>{section.title}</h2>{section.blocks ? <Blocks blocks={section.blocks} /> : <p>{section.text}</p>}</section>)}<footer><span>材料洞察與應用指南</span><a href="/articles">返回全部文章 ↗</a></footer></div>
        </div>
      </div>
    </article>
