    ...
    <article className="journal-sheet">
      ...
      <div className="journal-content">
        <div className="journal-hero">
          <div><span className="journal-category">{article.category}</span><h1>{article.title}</h1><p>{article.summary}</p><span className="journal-series">材料洞察 / 应用指南</span></div>
        ...
        <div className="journal-reading">
          ...
          <div className="journal-body">{article.sections.map((section, index) => <section id={`section-${index + 1}`} key={section.title}><h2><span>{String(index + 1).padStart(2, "0")}</span>{section.title}</h2><p>{section.text}</p></section>)}<footer><span>材料洞察与应用指南</span><a href="/articles">返回全部文章 ↗</a></footer></div>
        </div>
      </div>
    </article>
  </ArticleFrame>;
