...
export default function Blog({ listing = false }: { listing?: boolean }) {
  ...
  return <section className="material-blog shell" id="blog" aria-labelledby="blog-title">
    <header><div className="blog-heading-copy"><span className="blog-label">{listing ? "MATERIAL JOURNAL / 全部文章" : "06 — MATERIAL JOURNAL"}</span><h2 id="blog-title">材料洞察與應用指南</h2><p>從選材到成型，讓材料知識<br/>成為產品開發的下一步。</p></div>{!listing && <div className="blog-header-side"><div className="blog-actions"><div className="blog-pager" aria-label="切換文章"><button type="button" onClick={() => turnArticles(-1)} aria-label="上一篇文章"><span aria-hidden="true">‹</span></button><button type="button" onClick={() => turnArticles(1)} aria-label="下一篇文章"><span aria-hidden="true">›</span></button></div><div className="blog-more"><a href="/articles">查看更多文章 <span aria-hidden="true">›</span></a></div></div></div>}</header>
    <div className="blog-grid" aria-live={listing ? undefined : "polite"}>{visibleArticles.map(article => {
      const cardImage = (width: 640 | 1200) => articleAssetPath(article.image.replace(/-fast\.jpg$/, `-card-${width}.jpg`));
      return <article className="blog-card" key={article.slug}>
      <a className="blog-image" href={`/blog/${article.slug}`} aria-label={`閱讀文章：${article.title}`}><img src={cardImage(1200)} srcSet={`${cardImage(640)} 640w, ${cardImage(1200)} 1200w`} sizes="(max-width: 760px) calc(100vw - 32px), 33vw" alt={article.alt} loading={listing ? "lazy" : "eager"} decoding="async" width="1200" height="799"/></a>
