...
export default function ArticlesPage() {
  ...
  return <main className="article-hub">
    ...
    <div className="shell">
      ...
      <section className="article-hub-feature" aria-label="精选文章"><div><span>FEATURED / 精选阅读</span><h2><a href={`/blog/${featured.slug}`}>{featured.title}</a></h2><p>{featured.summary}</p><a className="article-hub-button" href={`/blog/${featured.slug}`}>阅读全文 ↗</a></div><a className="article-hub-cover" href={`/blog/${featured.slug}`} aria-label={`阅读：${featured.title}`}><img src={featured.image} alt={featured.alt} width="1536" height="1024" /></a></section>
      <section className="article-hub-all" aria-labelledby="all-articles"><header><h2 id="all-articles">全部文章</h2><span>{String(blogArticles.length).padStart(2,"0")} ARTICLES</span></header><div className="article-hub-grid">{blogArticles.map(article => <article key={article.slug}><a className="article-hub-thumbnail" href={`/blog/${article.slug}`} aria-label={`阅读：${article.title}`}><img src={article.image} alt={article.alt} width="640" height="400" loading="lazy" /></a><span>{article.category}</span><h3><a href={`/blog/${article.slug}`}>{article.title}</a></h3><p>{article.summary}</p><a className="article-hub-read" href={`/blog/${article.slug}`}>阅读全文 <span aria-hidden="true">↗</span></a></article>)}</div></section>
